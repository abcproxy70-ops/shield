...
- **v3.18.12** — HOTFIX для v3.18.11: исправлена двойная закрывающая `}` в `show_settings_menu` (внесена случайно в SH-NEW-125 patch). Из-за неё функция закрывалась раньше времени, `local` в остальной части выпадал в global scope → ошибка `local: can only be used in a function` при открытии меню Settings в guard CLI. Симптом был только в `[s] Settings`; install проходил OK. Других изменений нет.
