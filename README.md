...
- **Mobile-RU AS whitelist** (v3.16.2): relaxed limits (ct=1000, newconn=2000/min)
...
- **GitHub auto-sync** (v3.16.2): `lists/custom.txt` синкается с репо каждые 6ч.
  Локальные дополнения — в отдельном `custom-local.txt`, не перезаписываются.
- **Version check** (v3.16.2): нода раз в день проверяет github на новую версию,
...
- **v3.16.2** — убрана зависимость от MaxMind. `lists/mobile-ru.txt` теперь автогенерируется раз в неделю через GitHub Actions из публичного RIPEstat API. Все 13 mobile-RU AS проверены, ~2400 raw CIDR'ов → ~450 финальных после nft auto-merge. Mobile-RU стал обычным blocklist'ом через unified updater.
