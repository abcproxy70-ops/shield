...
- **v3.18.4** — security/data-loss hardening: (1) при interactive reinstall настройки оператора в shieldnode.conf (BLOCK_TOR, ENABLE_*, MAXMIND_LICENSE_KEY) теперь не теряются — merge-aware atomic write; (2) `guard upgrade` качает installer во временный файл с `-fsSL` и валидирует (shebang/version-marker/`bash -n`) ДО `exec` — на 404/MITM не убивает рабочую установку; (3) `.crowdsec_managed` marker — foreign CrowdSec не модифицируется (profiles.yaml, acquis.d, bouncer config, nft tables); (4) early-fail при <500MB/var, <50MB/etc, <50MB/tmp; (5) `prepare_seed_list` детектит header-only stub'ы и ретраится 3× с HTML-detection; (6) `guard rollback` — snapshot перед upgrade'ом, восстановление за одну команду; (7) BOUNCER_KEY валидируется regex'ом и пишется через awk вместо sed; (8) github-sync `mv` теперь same-FS atomic.
